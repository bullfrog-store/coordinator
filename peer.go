...
func (p *peer) putToZk(m *Modify) error {
	path := fmt.Sprintf("%s/%s", ZkKeyPrefix, string(m.Key))
	...
	if !exist {
		_, err = p.zkConn.Create(path, m.Value, ZkFlags, ZkAllAcl)
		return err
	}
	// if it exists,then send data to zk with the version
	_, err = p.zkConn.Set(path, m.Value, stat.Version)
	return err
}

func (p *peer) delToZk(m *Modify) error {
	path := fmt.Sprintf("%s/%s", ZkKeyPrefix, string(m.Key))
	// check if path exists
	exist, stat, _, err := p.zkConn.ExistsW(path)
	if err != nil {
